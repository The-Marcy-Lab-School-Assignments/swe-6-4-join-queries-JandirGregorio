...
const getAllBookmarksWithUsername = async () => {
  const query = `SELECT bookmarks.title, bookmarks.url, users.username
                ...
                ON users.user_id = bookmarks.user_id`;
...
const getBookmarksWithAllTags = async () => {
  const query = `SELECT bookmarks.title, bookmarks.url, tags.name AS tag_name
                FROM bookmarks
                INNER JOIN bookmark_tags
                ON bookmarks.bookmark_id = bookmark_tags.bookmark_id
                INNER JOIN tags
...
const getUsersWithBookmarkCount = async () => {
  const query = `SELECT users.username, COUNT(bookmarks.bookmark_id) AS total_bookmarks
                FROM users
                LEFT JOIN bookmarks
                ON users.user_id = bookmarks.user_id
